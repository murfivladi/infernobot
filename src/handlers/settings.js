...
const SETTING_MODAL_MAP = {
  ...
  set_welcome_msg:{ key: 'welcome_message', label: 'INPUT_WELCOME_MSG',title: 'BTN_SET_WELCOME_MSG' },
...
function buildSettingsFields(settings, locale, t) {
  ...
  return [
    ...
    { name: `💬 ${s('SETTINGS_WELCOME_MSG')}`,value: settings.welcome_message ? settings.welcome_message.substring(0, 50) + (settings.welcome_message.length > 50 ? '…' : '') : s('SETTINGS_NOT_SET'), inline: true },
...
function buildSettingsButtons(locale, t, Button) {
  return [
    ...
    new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId('set_welcome_msg').setLabel('💬 Msg').setStyle(ButtonStyle.Secondary),
